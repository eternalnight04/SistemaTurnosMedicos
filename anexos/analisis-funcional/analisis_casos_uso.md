# Análisis de Casos de Uso

Este documento centraliza los anexos de análisis funcional desarrollados para los casos de uso del sistema.

## Casos de Uso Documentados

- [Analisis Funcional - Crear Turno](../analisis-funcional/01-caso-de-uso-crear-turno.md)
- [Analisis Funcional - Reprogramar Turno](./02-caso-de-uso-reprogramar-turno.md)
- [Analisis Funcional - Cancelar Turno](./03-caso-de-uso-cancelar-turno.md)
- [Los Cuatro Pilares del Paradigma Orientado a Objetos](./pilares-poo.md)
- [Pseudocódigo - Happy Path Global del Sistema](./happy-path-global.md)

## Descripción

Los documentos referenciados incluyen:

- Descripción y trazabilidad con requisitos funcionales.
- Diagrama de casos de uso.
- Diagrama de actividades.
- Diagrama de secuencia.
- Diagrama de clases.
- Pseudocódigo orientado a objetos.

Cada anexo fue elaborado siguiendo la plantilla establecida en `00-plantilla-anexo-caso-de-uso.md`.
